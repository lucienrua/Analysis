...
\definecolor{ObsExe}{RGB}{150, 110, 210}      % 偏紫的色调
\definecolor{jiangmo}{HTML}{57A3F1}           % 原有的蓝% 
...
% #1: 环境命令名 (如 theorem)
% #2: 环境显示名 (如 Theorem)
% #3: 颜色变量名 (如 ObsTheorem)
% #4: 计数器规则 (如 numberwithin=section 或 sibling=theorem)
%\newcommand{\createobsbox}[4]{
%    % 1. 配置专属 thmtools 样式 (负责标题颜色、括号格式与换行)
%    \declaretheoremstyle[
%        headfont=\bfseries\color{#3},      % 主标题同色加粗
%        notefont=\bfseries\color{#3},      % 括号内的小标题同色加粗
%        notebraces={(}{)},                 % 小标题使用括号包裹 (匹配你图片中的效果)
%		bodyfont=\normalfont\color{black}, % 【新增】强制主体文字颜色
...
%        colback=#3!10,
%        % --- 强制颜色锁定开始 ---
%        colupper=black,              % 盒子内部文字主颜色
%        fontupper=\color{black},     % 强制字体颜色宏
%        before upper={\color{black}}, % 在文字开始前显式插入颜色指令
%        % 针对 breakable 的关键设置：确保每一页切开后都强制刷新颜色
%        extras broken={
%            colupper=black,
...
%    }
%    \tcolorboxenvironment{#1*}{
%        enhanced, breakable, rounded corners, boxrule=0.5pt,
%        colframe=#3, colback=#3!10,
%        left=1em, right=1em, top=0.5em, bottom=0.5em,
%        before skip=10pt, after skip=10pt,
%        separator sign none
%    }
...
\newcommand{\createobsbox}[4]{
    % 1. 配置专属 thmtools 样式
    \declaretheoremstyle[
        headfont=\bfseries\color{#3},      % 标题局部染色，没问题
        notefont=\bfseries\color{#3},      
        notebraces={(}{)},                 
        bodyfont=\normalfont,              % 【关键点 1】：绝对不要在这里写 \color{black}！
        spaceabove=0pt, spacebelow=0pt,
        headpunct={.},                     
        postheadspace=\newline             
    ]{#1style}
    ...
    \tcolorboxenvironment{#1}{
        ...
        colframe=#3,
        colback=#3!10,
        coltext=black,                     % 【关键点 2】：只用 tcolorbox 的原生属性控制文字颜色
        left=1em, right=1em, top=0.5em, bottom=0.5em,
        before skip=10pt, after skip=10pt,
        separator sign none
    }
...
% 批量生成所有环境，严格保留你原有的计数器依赖关系
\createobsbox{theorem}{Theorem}{ObsTheorem}{numberwithin=section}
\createobsbox{proposition}{Proposition}{ObsProp}{sibling=theorem}
\createobsbox{lemma}{Lemma}{ObsLemma}{sibling=theorem}
\createobsbox{corollary}{Corollary}{ObsCor}{sibling=theorem}
\createobsbox{claim}{Claim}{ObsClaim}{sibling=theorem}
\createobsbox{definition}{Definition}{ObsDef}{numberwithin=section}
\createobsbox{problem}{Problem}{jiangmo}{numberwithin=section}
\createobsbox{exercise}{Exercise}{ObsExe}{numberwithin=section}
\createobsbox{example}{Example}{ObsEg}{numberwithin=section}
\createobsbox{remark}{Remark}{ObsRemark}{numberwithin=section}
\createobsbox{note}{Note}{ObsNote}{numberwithin=section}
